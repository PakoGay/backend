...
public class ChildController {
    ...
    public PageResponse<ChildResponse> myChildren(@RequestParam(defaultValue = "0") int page,
        ...
        PageResponse<com.example.literacy.child.model.ChildProfile> response = childService.myChildren(currentUserFacade.currentUser(), page, size);
        return new PageResponse<>(response.content().stream().map(ChildResponse::from).toList(), response.page(), response.size(), response.totalElements(), response.totalPages());
    }
    ...
    public PageResponse<LeaderboardEntry> leaderboard(@RequestParam int age,
        ...
        return new PageResponse<>(response.content().stream().map(LeaderboardEntry::from).toList(),
                response.page(), response.size(), response.totalElements(), response.totalPages());
    }

    public record ChildRequest(@NotBlank @Size(max = 120) String name, @Min(3) @Max(8) int age, @NotBlank @Size(max = 100) String avatar, @Min(1) @Max(20) int startingLevel) {}
